...
function shouldUsePeriodicNotesSettings(periodicity) {
    ...
    const periodicNotes = window.app.plugins.getPlugin("periodic_note␥s");
...
function getDailyNoteSettings() {
    try {
        ...
        if (shouldUsePeriodicNotesSettings("daily")) {
            const { format, folder, template } = plugins.getPlugin("periodic_note␥s")?.settings?.daily || {};
            return {
...
function getWeeklyNoteSettings() {
    try {
        ...
        const periodicNotesSettings = pluginManager.getPlugin("periodic_note␥s")?.settings?.weekly;
...
/**
 * Read the user settings for the `periodic_note␥s` plugin
 * to keep behavior of creating a new note in-sync.
 */
function getMonthlyNoteSettings() {
    ...
    try {
        const settings = (shouldUsePeriodicNotesSettings("monthly") &&
            pluginManager.getPlugin("periodic_note␥s")?.settings?.monthly) ||
            {};
        return {
...
function getQuarterlyNoteSettings() {
    ...
    try {
        const settings = (shouldUsePeriodicNotesSettings("quarterly") &&
            pluginManager.getPlugin("periodic_note␥s")?.settings?.quarterly) ||
            {};
        return {
...
function getYearlyNoteSettings() {
    ...
    try {
        const settings = (shouldUsePeriodicNotesSettings("yearly") &&
            pluginManager.getPlugin("periodic_note␥s")?.settings?.yearly) ||
            {};
        return {
...
function appHasDailyNotesPluginLoaded() {
    ...
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    const periodicNotes = app.plugins.getPlugin("periodic_note␥s");
    return periodicNotes && periodicNotes.settings?.daily?.enabled;
}
/**
 * XXX: "Weekly Notes" live in either the Calendar plugin or the periodic_note␥s plugin.
...
const SETTINGS_UPDATED = "periodic_note␥s:settings-updated";
...
class PeriodicNotesPlugin extends obsidian.Plugin {
    ...
    configureCommands() {
        ...
        ["daily", "weekly", "monthly", "quarterly", "yearly"]
            ...
            // eslint-disable-next-line @typescript-eslint/no-explicit-any
            this.app.commands.removeCommand(`periodic_note␥s:${command.id}`));
        });
